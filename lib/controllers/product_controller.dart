...
class ProductController {
  Future<List<Map<String, String>>> fetchProducts() async {
    try {
      ...
      final response = await http.get(
        Uri.parse('http://192.168.203.46:8863/api/products/allProduct'),
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        if (decodedResponse is Map<String, dynamic> && decodedResponse['products'] is List<dynamic>) {
          ...
          const serverUrl = 'http://192.168.203.46:8863'; // 서버 URL 설정
  ...
  Future<Map<String, dynamic>> getProductInfoById(String productId) async {
    try {
      ...
      final response = await http.get(
        Uri.parse('http://192.168.203.46:8863/api/products/Product/$productId'),
        headers: {
          'Authorization': 'Bearer $token', // Bearer 토큰 추가
        },
      );
      ...
      if (response.statusCode == 200) {
        final decodedResponse = json.decode(response.body);
        const serverUrl = 'http://192.168.203.46:8863';
